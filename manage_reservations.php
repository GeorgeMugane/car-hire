...
<tr>
 <th>Registration No.</th>
  ...
  $result = mysqli_query($con, "SELECT COUNT(*) FROM reservations ORDER BY id DESC  ") or die("Count query error!");
  ...
  $result = mysqli_query($con, "SELECT * FROM reservations ORDER BY id
   DESC LIMIT $start, $max") or die("reservations order list query error!");
?>
  <?php
...
<a href="delete_res.php?id=<?php echo $row['id']; ?>" onclick="return confirm('Are you sure you want to approve this reservation?')" style="text-decoration:none; color:#808080;">Approve</a>
				</td>
                </tr>
 <?php
   }
